...
import { kBrands, Brand_Image } from 'constant/brand';
import GenS3Link from 'utils/GenS3Link';

const BoyStyleCustom = () => {
  ...
  useEffect(() => {
    if (user_id !== -1) {
      dispatch(kGetStyleCustom({ user_id, kid_count }));
    }
  }, [dispatch, user_id, kid_count]);
        ...
        <Grid item xs={12} md={viewState ? 8 : 12}>
          {viewState && (
            <Grid container>
              ...
              <Grid className="gray-border breadcrumb-tab" item xs={12} sm={3}>
                <Link to="/welcome/style-custom/kids/boys" style={{ color: '#ff6c00' }}>
