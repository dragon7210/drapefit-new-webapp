...
const ImageSelectorCheckboxAdmin = (props) => {
  return (
    <>
      <Grid container>
        <Grid item xs={12}>
          <FormControl fullWidth>
            ...
            <Grid container spacing={1} className="custom-hidden-image-checkbox">
              {props.content.map((item, index) => (
                <Grid key={index} item xs={4} sm={2.4}>
                  <Typography className="ellipsis-nowrap">{item.title}</Typography>
                  <FormControlLabel
                    control={
                      <Checkbox
...
ImageSelectorCheckboxAdmin.propTypes = {
...
export default ImageSelectorCheckboxAdmin;
