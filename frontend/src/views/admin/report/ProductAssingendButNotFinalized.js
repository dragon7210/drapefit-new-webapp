...
import { getProductAssingendButNotFinalized } from 'actions/admin/report';
...
const ProductAssingendButNotFinalized = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getProductAssingendButNotFinalized());
  }, [dispatch]);

  const { productAssingendButNotFinalized } = useSelector((state) => state.report);
  let updateData = productAssingendButNotFinalized.map((item) => {
    return {
      ...item,
...
export default ProductAssingendButNotFinalized;
