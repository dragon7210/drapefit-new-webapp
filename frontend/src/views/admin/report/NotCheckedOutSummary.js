...
import { getNotCheckedOutSummary } from 'actions/admin/report';
...
const NotCheckedOutSummary = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getNotCheckedOutSummary());
  }, [dispatch]);

  const { notCheckedOutSummaryList } = useSelector((state) => state.report);
  console.log(notCheckedOutSummaryList);
  let updateData = [];
  notCheckedOutSummaryList.map((item) => {
    let keepCount = 0;
    let exchangeCount = 0;
    let returnCount = 0;
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Not Checkout Summary</Typography>
...
export default NotCheckedOutSummary;
