...
import { getMonthlyProductNotReturned } from 'actions/admin/report';
...
const MonthlyProductNotReturned = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getMonthlyProductNotReturned());
  }, [dispatch]);

  const { monthlyProductNotReturnedList } = useSelector((state) => state.report);
  let updateData = [];
  updateData = monthlyProductNotReturnedList.map((item) => {
    return {
      ...item,
      user: item?.user?.name,
...
export default MonthlyProductNotReturned;
