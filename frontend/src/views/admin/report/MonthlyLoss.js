...
import { getMonthlyLoss } from 'actions/admin/report';
...
const MonthlyLoss = () => {
  ...
  useEffect(() => {
    dispatch(getMonthlyLoss());
  }, [dispatch]);

  const { monthlyLossList } = useSelector((state) => state.report);
  let updateData = monthlyLossList.map((item) => {
    return {
      ...item,
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Loss</Typography>
