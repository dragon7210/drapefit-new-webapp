...
import { getNotCheckedOutDetails } from 'actions/admin/report';
...
const NotCheckedOutDetails = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getNotCheckedOutDetails());
  }, [dispatch]);

  const { notCheckedOutDetailsList } = useSelector((state) => state.report);
  let updateData = [];
  notCheckedOutDetailsList.map((item) => {
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Not Checked Out Detail</Typography>
...
export default NotCheckedOutDetails;
