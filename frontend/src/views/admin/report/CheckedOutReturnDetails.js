...
import { getCheckedOutReturnDetails } from 'actions/admin/report';
...
const CheckedOutReturnDetails = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getCheckedOutReturnDetails());
  }, [dispatch]);

  const { checkOutReturnDetailList } = useSelector((state) => state.report);
  let updateData = checkOutReturnDetailList.map((item) => {
    let status;
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Checkedoit Return Detail</Typography>
...
export default CheckedOutReturnDetails;
