...
import { getCheckedOutProductDetail } from 'actions/admin/report';
...
const CheckedOutProductDetail = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getCheckedOutProductDetail());
  }, [dispatch]);

  const { checkedOutProductDetailList } = useSelector((state) => state.report);
  let updateData = checkedOutProductDetailList.map((item) => {
    let Products = 1;
    return {
      ...item,
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Customer checked out with product details</Typography>
...
export default CheckedOutProductDetail;
