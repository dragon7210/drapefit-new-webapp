...
import { getMonthlyclientConsumed } from 'actions/admin/report';
...
const MonthlyclientConsumed = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getMonthlyclientConsumed());
  }, [dispatch]);

  const { monthlyclientConsumedList } = useSelector((state) => state.report);
  let updateData = [];
  updateData = monthlyclientConsumedList.map((item) => {
    return {
      ...item,
      user: item?.user?.name,
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Monthly client consumed</Typography>
...
export default MonthlyclientConsumed;
