...
import { getCheckedOutNotReturnDetails } from 'actions/admin/report';
...
const CheckedOutNotReturnDetails = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(getCheckedOutNotReturnDetails());
  }, [dispatch]);

  const { checkedOutNotReturnDetailsList } = useSelector((state) => state.report);
  let updateData = checkedOutNotReturnDetailsList.map((item) => {
    let status;
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Checkout Not Return Details</Typography>
...
export default CheckedOutNotReturnDetails;
