...
import { Button, Box, Typography, Grid, MenuItem, Select, InputLabel, FormControl } from '@mui/material';
...
import { getFinalizedSummaryReport } from 'actions/admin/masterSummary';
...
const FinalShippedSmRpt = () => {
  ...
  const handleFilter = () => {
    dispatch(getFinalizedSummaryReport({ profile, startDate, endDate }));
  };

  const { finalizedSummaryReport } = useSelector((state) => state.masterSummary);
  let updateData = [];
  updateData = finalizedSummaryReport.map((item) => {
    return {
      ...item,
...
export default FinalShippedSmRpt;
