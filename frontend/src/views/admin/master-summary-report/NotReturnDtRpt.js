...
import { getNotReturnDetailReport } from 'actions/admin/masterSummary';
...
const NotReturnDtRpt = () => {
  ...
  const handleFilter = () => {
    dispatch(getNotReturnDetailReport({ profile, startDate, endDate }));
  };
  const { notReturnDetailReport } = useSelector((state) => state.masterSummary);
  let updateData = [];
  notReturnDetailReport.map((item) => {
    let Img;
    item?.products?.map((a) => {
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Not Return Details</Typography>
