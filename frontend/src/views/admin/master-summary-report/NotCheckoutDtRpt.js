...
import { getNotCheckoutDetailReport } from 'actions/admin/masterSummary';
...
const NotCheckoutDtRpt = () => {
  ...
  const handleFilter = () => {
    dispatch(getNotCheckoutDetailReport({ profile, startDate, endDate }));
  };

  const { notCheckoutDetailReport } = useSelector((state) => state.masterSummary);
  let updateData = [];
  notCheckoutDetailReport.map((item) => {
    let Img;
    item?.products?.map((a) => {
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Not Checkout Details</Typography>
...
export default NotCheckoutDtRpt;
