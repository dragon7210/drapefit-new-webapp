...
import { getReturnProcessSummaryReport } from 'actions/admin/masterSummary';
...
const ReturnProcessedSmRpt = () => {
  ...
  const handleFilter = () => {
    dispatch(getReturnProcessSummaryReport({ profile, startDate, endDate }));
  };

  const { returnProcessSummaryReport } = useSelector((state) => state.masterSummary);
  let updateData = [];
  updateData = returnProcessSummaryReport.map((item) => {
    return {
      ...item,
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Return Processed Summary</Typography>
...
export default ReturnProcessedSmRpt;
