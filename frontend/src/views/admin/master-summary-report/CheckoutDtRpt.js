...
import { getCheckoutDetailReport } from 'actions/admin/masterSummary';
...
const CheckoutDtRpt = () => {
  ...
  const handleFilter = () => {
    dispatch(getCheckoutDetailReport({ profile, startDate, endDate }));
  };

  const { checkoutDetailReport } = useSelector((state) => state.masterSummary);
  let updateData = [];
  checkoutDetailReport.map((item) => {
    let Img;
    item?.products?.map((a) => {
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Product Checkout Details</Typography>
