...
import { getReturnProcessDetailReport } from 'actions/admin/masterSummary';
...
const ReturnProcessedDtRpt = () => {
  ...
  const handleFilter = () => {
    dispatch(getReturnProcessDetailReport({ profile, startDate, endDate }));
  };

  const { returnProcessDetailReport } = useSelector((state) => state.masterSummary);
  let updateData = [];
  returnProcessDetailReport.map((item) => {
    let Img;
    item?.products?.map((a) => {
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Return Processed Details</Typography>
...
export default ReturnProcessedDtRpt;
