...
import { getNotRtExDetailReport } from 'actions/admin/masterSummary';
...
const NotRtExDtRpt = () => {
  ...
  const handleFilter = () => {
    dispatch(getNotRtExDetailReport({ profile, startDate, endDate }));
  };

  const { notRtExDetailReport } = useSelector((state) => state.masterSummary);
  let updateData = [];
  notRtExDetailReport.map((item) => {
    let Img;
    item?.products?.map((a) => {
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Not Return Exchange Details</Typography>
