...
import { getDeclineDetailReport } from 'actions/admin/masterSummary';
...
const DeclineDtRpt = () => {
  ...
  const handleFilter = () => {
    dispatch(getDeclineDetailReport({ profile, startDate, endDate }));
  };

  const { declineDetailReport } = useSelector((state) => state.masterSummary);
  let updateData = [];
  declineDetailReport.map((item) => {
    let Img;
    item?.products?.map((a) => {
  ...
  return (
    <>
      <Box className="admin-page-title-part">
        <Typography className="page-title">Decline Details</Typography>
