...
import { getInventoryProfile, editInventoryProfile } from 'actions/inventory/product';
import DFnewLogger from 'utils/DFnewLogger';

const Profile = () => {
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { profile } = useSelector((state) => state.invProduct);
      ...
      <Dialog open={openEditConfirm} onClose={handleEditConfirmDialog}>
        ...
        <DialogActions>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                await dispatch(editInventoryProfile(profileInput));
                handleEditConfirmDialog();
              } catch (err) {
